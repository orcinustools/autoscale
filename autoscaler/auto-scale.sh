...
default_scale () {
  service_name=$1
  auto_scale_label=$(docker service inspect $service_name | jq '.[].Spec.Labels["autoscaler"]')
  replica_minimum=$(docker service inspect $service_name | jq '.[].Spec.Labels["autoscaler.minimum"]' | sed 's/\"//g')
  replica_maximum=$(docker service inspect $service_name | jq '.[].Spec.Labels["autoscaler.maximum"]' | sed 's/\"//g')
